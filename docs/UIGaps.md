...
| Disbursement Checklist | ✅ | `OfferAcceptanceTab.razor` — CP/CS item tables, role-filtered actions, single-modal pattern, CP gate enforced at domain level (Session 46); Disbursement Memo card with system PDF download + countersigned copy upload (Session 56) |
...
| 6.9 | 2026-05-22 | Sessions 60–61 (namp-crms branch): NAMP module — (1) StandingCommittee LocationId added (migration + admin UI). (2) AgroServiceCompany tech appraisal report fields extended. (3) Pre-deployment gate checklist — `NampPreDeploymentChecklistTemplate`+`NampPreDeploymentChecklistItem`; admin CRUD page `/admin/namp-predeploy-checklist`; gate enforcement on BeginPreDeployment; repair endpoint `POST /api/seed/namp-repair-checklist`; 4 default items seeded. (4) NAMP Offer Letter PDF — `NampOfferLetterPdfGenerator` with Fineract amortisation table; Detail.razor Offer Letter tab (download + countersigned upload); OfferAccepted/OfferLapsed buttons. (5) NAMP Loan Pack PDF — `NampLoanPackPdfGenerator` 12-section on-demand PDF; download button for relevant roles. (6) NAMP AI Advisory — `NampAdvisory` entity + `NampViabilityScoreConfig`; 5-category weighted advisory (4 AI + TechnicalViability synthetic); AI Advisory tab in Detail.razor; admin page `/admin/namp-viability`; 3 default viability configs seeded. No corporate UI changes. Critical NAMP gap: Fineract loan creation at deployment still not implemented. |
| 6.8 | 2026-05-11 | Session 58: Tutorial credential fixes (all short usernames → email addresses; password corrected to `Password1$$$`). Scalar API UI added to `CRMS.API` (`/scalar/v1`). Help guide (`Help/Index.razor`) comprehensively rewritten — all 13 roles, full 17-stage workflow, all tabs/features accurately documented; role-aware (sidebar nav filtered + direct URL access blocked via `CanViewSection()` + `RenderAccessDenied()`). Build: 0 errors. |
| 6.7 | 2026-05-06 | Session 57: `DisbursementBranchApproval` stage removed — flow is now `DisbursementPending` (Operations) → `DisbursementHQApproval` (GMFinance) → `Disbursed`. Workflow DB transition fix: `PrepareDisbursementMemo()` now targets `DisbursementHQApproval`; seeder upgrade deletes old branch transition and inserts new Approve + Return transitions. Domain-workflow desync repair: three idempotent SQL statements in seeder reset any apps stuck at `DisbursementBranchApproval` on startup. Build: 0 errors. |
| 6.6 | 2026-05-06 | Session 56 (cont): Return from `DisbursementPending` → `SecurityPerfection` added for Operations role. Full stack: domain method, command+handler, DI, `ApplicationService.ReturnApplicationAsync`, `ShowReturnButton` in `Detail.razor`. Build: 0 errors. |
| 6.5 | 2026-05-05 | Session 56: `OfferAcceptanceTab` — "Upload Countersigned Copy" button now visible to Operations/BranchApprover/SystemAdmin at disbursement stages. Root fix: `CanUploadSignedMemo` moved from child component to `Detail.razor` and passed as `[Parameter]` — eliminates Blazor Server auth-timing issue in child tabs. Pattern now consistent with all other permission flags (`CanManageValuation`, `CanVerifyStatements`, etc.). |
| 6.4 | 2026-05-03 | Session 53: `ViewCollateralModal.razor` — collateral document list now shows document type as prominent coloured uppercase label; description shown below filename when present. Valuation section shows `IndicativeValue` above the formal valuation grid with "Entered at registration — pending formal valuation" note. `CheckApprovalGateQuery` bug fix: `Valued` collateral no longer blocks gate at `CreditAnalysis` stage (only `Proposed`/`UnderValuation` block); `FinalApproval` still requires full approval. AI Advisory crash fix: `Math.Round(decimal, -3)` (invalid precision) → `/ 1000m * 1000m` pattern in both `RuleBasedScoringEngine.cs` and `MockAIAdvisoryService.cs`; failed advisory DB records deleted to restore Generate button. Full application status chain confirmed in `AppStatus.cs` and `LoanApplicationStatus` enum including `LegalReview`, `LegalApproval`, `SecurityPerfection`, `SecurityApproval`, `DisbursementBranchApproval`, `DisbursementHQApproval`. |
| 6.3 | 2026-05-04 | Session 52: Bureau/credit check bug fixes (business card always renders; `CanRerunCreditChecks` detects absent reports; SmartComply RC prefix normalization; idempotency gate extended for missing reports). `CheckApprovalGateQuery` CreditAnalysis now gates on `AnalysisStatus` for bank statements. Collateral `IndicativeValue` mapped and displayed. `RefetchInternalBankStatementCommand` — delete + re-fetch from CBS (6 months from today, temporarily; reverts to `CreatedAt` when CBS supports backdating); `StatementsTab` "Refresh from Core Banking" button with disable-on-click spinner. Guarantor approval design discussed and confirmed (Credit Officer owns it; legal deed sign-off deferred). |
| 6.2 | 2026-04-30 | Session 51: Approval Gate feature fully implemented — `ApprovalGateModal` (hard block / override note / soft warning modes), gate check wired to Approve button in `Detail.razor`, override banners above tabs, inline rejection notes/verification notes on all 5 item tabs (Documents, Statements, Financials, Collateral, Guarantors), FinancialStatement Verify/Reject buttons per year, `ApprovalOverrideRecord` entity + repository + EF migration, `CheckApprovalGateQuery` + `SaveApprovalOverrideCommand` handlers. Approval Gate removed from pending section — now complete. |
| 6.1 | 2026-04-29 | Session 50 (continued): `CanManageGuarantors` in `Detail.razor` now role-gated (CreditOfficer/CreditManager/HOReviewer/RiskManager/FinalApprover/SystemAdmin) — Loan Officers could see Approve/Reject buttons on guarantors post-submission (red Cancel icon visually similar to Delete). Approval Gate feature: full design deliberated and approved — per-stage configurable strict/flexible gate, `ApprovalOverrideRecord` entity, inline rejection notes on all item types, persistent override banners; no code written yet. New pending feature in Section 2. |
| 6.0 | 2026-04-29 | Session 50: Configurable `CollateralTypeConfig` full stack — `CollateralTypeConfigs` table, 11 seeded default types (0–50% haircut, MarketValue/FSV basis), `CollateralTypeConfigRepository`, 4 handlers + 1 repository registered in DI, `CollateralTypes.razor` admin page (`/admin/collateral-types`) with CRUD. `AddCollateralModal` rewritten — dynamic type dropdown from DB, haircut info callout, indicative value only (formal valuation moved to Credit Officer flow). `SetCollateralValuationModal` updated — valuation basis selector, valuer name/company, AcceptableValue auto-computed. `CanManageValuation` in `Detail.razor` now role-gated (CreditOfficer/CreditManager/HOReviewer/SystemAdmin only) — Loan Officers no longer see Set Valuation or valuation upload buttons during review. Bug fix: 5 missing DI handler registrations caused silent empty-list returns (caught by try/catch in `ApplicationService`). |
| 5.9 | 2026-04-14 | Session 48: Stale-state fix — `StateHasChanged()` added to `LoadApplication()` in `Detail.razor`; covers all 20+ action handlers (CastVote, ExecuteAction, VerifyDocument, VerifyStatement, AnalyzeStatement, etc.) uniformly. Document viewer modal height increased to `height: 92vh; max-height: 92vh`; body replaced `height: 70vh` with `flex: 1; min-height: 0` to fill available space between header and footer. `SetupCommitteeModal`: added `show` class to `.modal-backdrop`, nested `.modal` inside backdrop (was a sibling), added `@onclick:stopPropagation`, added missing closing `</div>` — modal now renders as centred overlay. Business proposal: `docs/CRMS_Proposal.md` (24 PPT-ready slides with `---` separators) and `docs/CRMS_Proposal.html` (standalone HTML with embedded CSS, animated score bars, full lifecycle flow, role cards, implementation timeline). |
| 5.8 | 2026-04-13 | Session 47: Offer Acceptance modal fields: KFS acknowledgement checkbox, Acceptance Method dropdown (InBranchSigning/Courier/Electronic), CustomerSignedAt date picker — CBN CP-compliant; `SystemAdmin` removed from acceptor role (Operations only). Evidence viewer in `OfferAcceptanceTab`: view button opens 92vh iframe modal at `/api/documents/{id}/view`. Loan Pack PDF full data: approved terms in Executive Summary, bureau Active Facilities table + Delinquent Accounts table per report, new Committee Decision section (vote tally, recommended terms, per-member vote table). Loan Pack button `CanGeneratePack`: now visible at all active statuses (excludes Draft/Rejected/Cancelled/Closed) to LoanOfficer/CreditOfficer/HOReviewer/BranchApprover/FinalApprover/Operations/SystemAdmin/committee members. Build: 0 errors. |
