...
**Status:** Phase 1 COMPLETE (Backend + UI) | Audit Fixes Complete (A-E) | UI Enhancements COMPLETE | SmartComply Integration COMPLETE (Backend + UI) | Bank Statement UI COMPLETE | AI Advisory Data Quality COMPLETE | Scoring Config Editor COMPLETE | Core Banking API COMPLETE | Location Hierarchy COMPLETE | Location Bug Fixes COMPLETE | Location Admin UI COMPLETE | UI Wiring Audit + Committee Setup COMPLETE | Standing Committee + Auto-Routing COMPLETE | P3 UI Gaps RESOLVED | Hybrid AI Advisory COMPLETE | Save Transactions Bug Fixed | Add Transactions UX Bugs Fixed | Submit for Review Bug Fixed | Workflow Save Bugs Fixed (Approve/Return/AutoTransition) | Lifecycle Gaps G4/G6/G7/G9/G10/G11 Fixed | CreditAnalysis Stage Fully Wired | HOReview→CommitteeCirculation Desync Fixed | Disbursement Checklist COMPLETE (CP/CS state machine + PDF + CS monitoring) | Offer Acceptance Operational Fields COMPLETE | Loan Pack PDF Full Data COMPLETE | Stale-State Audit Fixed | Modal Bugs Fixed | Business Proposal COMPLETE | Approval Gate COMPLETE | Bureau+CreditCheck Bug Fixes COMPLETE | Internal Statement Re-fetch COMPLETE | Collateral Document Description + IndicativeValue UI COMPLETE | Approval Gate Collateral Bug Fixed | AI Advisory Math.Round Crash Fixed | Disbursement Memo Countersigned Upload Bug Fixed | DisbursementPending Return Path COMPLETE | DisbursementBranchApproval Removed + Workflow DB Fixed + Domain-Workflow Desync Repair COMPLETE | Scalar API UI ADDED | Help Guide Role-Aware Rewrite COMPLETE | NAMP Technical+Financial Appraisal COMPLETE (doc management + decision modals) | NAMP Pre-Deploy Checklist COMPLETE | NAMP Offer Letter + Loan Pack COMPLETE | NAMP AI Advisory COMPLETE | NAMP Fineract Loan Booking COMPLETE | NAMP Loan Product Admin Bug Fixes COMPLETE | NAMP Portal Status API COMPLETE | Phase 2 Pending (Retail)
...
| 8.2 | 2026-05-19 | Factory AI | Session 59 (addendum): Legacy seeder account fix — `@crms.test` users created by `SeedData.cs` were Status=0 and hashed with `Test@123`; `ComprehensiveDataSeeder` username-match skipped them entirely. Extended fix block in `ComprehensiveDataSeeder.cs` to target any known username that is inactive or has a non-mocked hash: calls `SetPasswordHash("Password1$$$")` + `Activate()` on startup. Effective login email table added to SESSION_HANDOFF Section 8. Build: 0 errors. |
| 8.1 | 2026-05-19 | Factory AI | Session 59: NAMP Detail page — document management + appraisal decision modals. (1) New commands: `DeleteNampDocumentCommand`, `UpdateNampDocumentCommand`. (2) Domain: `NampDocument.Create()` made public; `UpdateMetadata()` added; `NampApplication.UploadDocument()` returns `NampDocument`; `RemoveDocument()` added. (3) Repository: `AddNampDocumentAsync` (explicit DbSet.Add, fixes EF Modified→Added tracking bug for non-default GUID keys) + `GetNampDocumentByIdAsync`. (4) `Program.cs`: inline document view endpoint `GET /api/namp-documents/{id}/view`. (5) ApplicationService: `UploadNampDocumentAsync`, `DeleteNampDocumentAsync`, `UpdateNampDocumentAsync`, `SaveNampTechnicalAppraisalReportAsync`, `SaveNampFinancialAppraisalReportAsync`. (6) Detail.razor: View modal (iframe/img/fallback, z-index:1100), Edit modal (existing file card + optional replace), Delete confirm modal, `GetDocIcon()` helper. (7) Appraisal decision moved from inline tab cards to header-button-triggered modals — consistent with all other workflow actions on the page; read-only decision record cards retained in tabs. Build: 0 errors. |
| 8.0 | 2026-05-11 | Factory AI | Session 58: Tutorial credential fixes — all short usernames replaced with email addresses in `Tutorial_LoanLifecycle.html`; password corrected to `Password1$$$` (confirmed from seeder). Scalar API UI added — `Scalar.AspNetCore` v2.14.11 added to `CRMS.API.csproj`; `app.MapScalarApiReference()` in `Program.cs`; accessible at `/scalar/v1` in Development. Help guide comprehensive rewrite — `Help/Index.razor` updated to accurately document all 13 roles, full 17-stage workflow, all tabs/features/stages; role-aware: sidebar nav filtered by role; direct URL access blocked via `CanViewSection()` switch + `RenderAccessDenied()` fragment; new role guides for HOReviewer, LegalOfficer, HeadOfLegal, GMFinance; Approval Gates, Offer Acceptance tab, Admin Collateral Types documented. Build: 0 errors. |
| 7.9 | 2026-05-06 | Factory AI | Session 57: (1) `DisbursementBranchApproval` stage removed — flow is now `DisbursementPending` (Operations) → `DisbursementHQApproval` (GMFinance). Removed from `ApplicationService.cs` approve/return paths, `Detail.razor` (5 places), `Index.razor` filter + badge maps, `OfferAcceptanceTab.razor` `ShowMemoCard`, `ComprehensiveDataSeeder.cs` transition lists. (2) Workflow DB transition fix — `PrepareDisbursementMemo()` domain method now targets `DisbursementHQApproval`; seeder upgrade block deletes old `DisbursementPending→DisbursementBranchApproval` transition, inserts `DisbursementPending→DisbursementHQApproval` (Approve, Operations) and `DisbursementPending→SecurityPerfection` (Return, Operations). (3) Domain-workflow desync data repair — seeder runs 3 idempotent SQL statements on startup: advance `WorkflowInstances.CurrentStatus` from `DisbursementBranchApproval` → `DisbursementHQApproval`; sync `LoanApplications.Status` for case A (seeded apps); reset `LoanApplications.Status` for case B (stuck apps where domain write persisted before workflow transition failed). Root cause of stuck apps: domain command write and workflow transition are not wrapped in a shared transaction — a failed transition leaves domain status ahead of workflow status, making apps invisible to all role queues. Build: 0 errors. |
| 7.8 | 2026-05-06 | Factory AI | Session 56 (cont): Return from DisbursementPending feature. Operations role can now return to `SecurityPerfection` when security documents are insufficient for disbursement. Full stack: `ReturnFromDisbursementPending()` domain method on `LoanApplication`; `ReturnFromDisbursementPendingCommand` + `ReturnFromDisbursementPendingHandler` in `SubmitLoanApplicationCommand.cs`; handler registered in `DependencyInjection.cs`; `ReturnApplicationAsync` updated with `"DisbursementPending" → SecurityPerfection` in `targetStatus` switch + domain handler block; `ShowReturnButton` in `Detail.razor` now includes `DisbursementPending → Operations`. Management holds (no system-level hold state needed) handled by Operations simply not clicking Approve. Build: 0 errors. |
| 8.4 | 2026-06-03 | Factory AI | Session 67: NAMP Portal Status API. `GET /api/v1/namp/webhook/application/{applicationReference}` added to `NampWebhookController` (same `X-Api-Key` auth, no new controller). Always 200 — friendly `found: false` payload when not found. 7 portal-facing statuses mapped from all internal `NampApplicationStatus` values. Decline reason from stage-specific note fields. Timeline from `NampStatusHistory`. Fineract enrichment conditional by status: `CalculateRepaymentScheduleAsync` for OfferMade/Processing (monthly installment + totals); `GetLoanDetailAsync` for Active/Closed (balance, next installment, arrears); all calls try/caught with `fineractDataAvailable` flag. Files: `INampApplicationRepository` + `NampApplicationRepository` (new `GetByApplicationReferenceWithHistoryAsync`); `NampWebhookController` (+`INampApplicationRepository` injection, GET endpoint, 4 helper methods, 7 response DTO classes). Build: 0 errors. |
| 8.3 | 2026-05-22 | Factory AI | Sessions 60–61 (namp-crms branch): (1) StandingCommittee LocationId — FK added to entity, EF config, repository, commands, DTOs, Committees.razor admin page; migration `AddStandingCommitteeLocationId`. (2) AgroServiceCompany tech appraisal fields — `NampTechnicalAppraisalReport` expanded with site visit, soil test, irrigation, yield projection fields; migration `AddCompanyTechAppraisalFields`. (3) Pre-deployment gate checklist — `NampPreDeploymentChecklistTemplate`+`NampPreDeploymentChecklistItem` entities, EF configs, repository, commands, admin page `/admin/namp-predeploy-checklist`, gate enforcement on BeginPreDeployment; migrations `AddPreDeploymentGateChecklist`+`AddNampPreDeploymentChecklist`; seeder seeds 4 default items; `POST /api/seed/namp-repair-checklist` endpoint added. (4) NAMP Offer Letter PDF — `INampOfferLetterPdfGenerator`+`NampOfferLetterPdfGenerator` (QuestPDF + Fineract amortisation table); Offer Letter tab in Detail.razor (download + countersigned upload, SignedNampOfferLetter enum value 7); OfferAccepted/OfferLapsed buttons. (5) NAMP Loan Pack PDF — `INampLoanPackGenerator`+`NampLoanPackPdfGenerator` (on-demand 12-section PDF, no storage); `GenerateNampLoanPackHandler`; download button for relevant roles from any post-Draft status. (6) NAMP AI Advisory — `NampAdvisory` entity (JSON-stored 5-category scores), `NampViabilityScoreConfig` (admin-configurable Viable/Marginal/NotViable→score+weight); `GenerateNampAdvisoryHandler` reuses `IAIAdvisoryService` for 4 categories + adds TechnicalViability as 5th synthetic via weighted average; `AdditionalContext` added to `AIAdvisoryRequest`; `GetNampAdvisoryHandler`, `UpdateNampViabilityScoreConfigHandler`, `GetNampViabilityScoreConfigsHandler`; migration `AddNampAdvisory`; seeder seeds 3 viability configs; AI Advisory tab in Detail.razor (score grid, narrative); admin page `/admin/namp-viability`; NavMenu updated. Security: OpenAI API key was committed to appsettings.json, caught by GitHub push protection, amended out and key cleared (action: rotate key). Build: 0 errors. 5 migrations. Committed as 5d44c81. |
| 7.7 | 2026-05-05 | Factory AI | Session 56: Bug fix — `OfferAcceptanceTab.razor` "Upload Countersigned Copy" button not appearing for Operations role at `DisbursementPending`. Root cause: `CanUploadSignedMemo` computed inside child component read `AuthService.CurrentUser` which is null during initial Blazor Server render cycle in child tabs. Fix: moved computation to `Detail.razor` (where auth state is confirmed working) and passed as `[Parameter] public bool CanUploadSignedMemo` — consistent with existing pattern for `CanManageValuation`, `CanVerifyStatements`, etc. Also: killed locked CRMS.API process (PID 24588) to unblock full solution build. Build: 0 errors. |
| 7.6 | 2026-05-03 | Factory AI | Session 53: (1) `CollateralDocumentDto` — `Description` field added; all 3 mapping sites in `CollateralCommands.cs` + `CollateralQueries.cs` updated; `CollateralDocumentInfo` in `ApplicationServiceDtos.cs` updated; `ApplicationService.GetCollateralDetailAsync` mapping updated. (2) `ViewCollateralModal.razor` — document type rendered as prominent coloured uppercase label with Verified badge inline; description rendered below filename; `IndicativeValue` displayed above formal valuation grid with registration note. (3) `CheckApprovalGateQuery` collateral bug fix: split by stage — `Valued` passes at `CreditAnalysis`; only `Proposed`/`UnderValuation` block; `FinalApproval` still blocks on `Valued`. (4) AI Advisory `Math.Round(decimal, -3)` crash fixed in `RuleBasedScoringEngine.cs` (line 606) and `MockAIAdvisoryService.cs` (line 644) — decimal overload disallows negative precision; pattern changed to `/ 1000m * 1000m`; failed `CreditAdvisories` DB records deleted to restore Generate button visibility. (5) Full workflow status chain verified in `AppStatus.cs` + `LoanApplicationStatus` enum: confirmed `LegalReview`, `LegalApproval`, `SecurityPerfection`, `SecurityApproval`, `DisbursementPending`, `DisbursementBranchApproval`, `DisbursementHQApproval` all exist. Build: 0 errors. |
| 7.5 | 2026-05-04 | Factory AI | Session 52: Bug fixes + re-fetch feature. (1) `BureauTab.razor` — business report section always renders; placeholder shown when no report. (2) `Detail.razor` `CanRerunCreditChecks` extended to detect absent individual/business bureau reports. (3) `SmartComplyProvider.GetBusinessCreditReportAsync` — RC prefix normalization (bare numeric → "RC" prefix). (4) `ProcessLoanCreditChecksCommand` idempotency gate extended with `hasMissingIndividualReport` + `hasMissingBusinessReport` guards. (5) `CheckApprovalGateQuery` — `CreditAnalysis` added to `checkBankStatements` scope; check uses `AnalysisStatus != Completed` at that stage. (6) `CollateralInfo.IndicativeValue` mapped in `ApplicationService.GetCollateralsForApplicationAsync`; `CollateralTab` shows indicative value with "Indicative — pending valuation" label. (7) `RefetchInternalBankStatementCommand` — new Application layer command; deletes CoreBanking statement, re-fetches 6 months from today via CBS; handler registered in DI; `ApplicationService.RefetchInternalBankStatementAsync`; `StatementsTab` "Refresh from Core Banking" button with `isRefetching` disable+spinner guard; `Detail.razor` `RefetchInternalStatement()` wired via `OnRefetch`. Design discussion: guarantor approval confirmed as Credit Officer responsibility; legal deed sign-off deferred. Build: 0 errors (Application project). |
| 7.4 | 2026-04-30 | Factory AI | Session 51: Approval Gate feature fully implemented. Config: `WorkflowApprovalGates` in `appsettings.json`; `WorkflowApprovalGateSettings`/`ApprovalGateConfig`/`IApprovalGateConfig`. Domain: `ApprovalOverrideRecord` entity; `OverrideRecords` collection on `LoanApplication`; `IApprovalOverrideRepository`; EF config + migration `AddApprovalOverrideRecords`. Application: `CheckApprovalGateQuery` (scoped per stage — Documents+Statements+Financials for BranchReview/HOReview, Collateral+Guarantors for CreditAnalysis, all types for FinalApproval); `SaveApprovalOverrideCommand`+`GetApprovalOverridesHandler`; `RejectFinancialStatementCommand`+handler; `RejectionReason` added to `FinancialStatementSummaryDto`+`GuarantorSummaryDto`; `VerificationNotes` added to `BankStatementSummaryDto`; all DTO mappers updated. DI: all new handlers + `IApprovalGateConfig` + `WorkflowApprovalGateSettings` registered. UI: `ApprovalGateModal.razor` (3 modes: hard block / mandatory override note / soft warning); `Detail.razor` — `HandleApproveClick` gate check, override banners above tabs, financial verify/reject wiring, override records loaded on page load; `DocumentsTab`/`StatementsTab`/`FinancialsTab`/`CollateralTab`/`GuarantorsTab` each show inline rejection reason/notes under Rejected badge. `ApplicationService` + 5 new methods. `ApplicationModels.cs` + `ApprovalGateResultModel`/`GateItemModel`/`ApprovalOverrideInfo` + new fields on existing models. Build: 0 errors. |
| 7.3 | 2026-04-29 | Factory AI | Session 50 (continued): Two fixes + one major design decision. Fix 1: `CanManageGuarantors` in `Detail.razor` now role-gated (CreditOfficer/CreditManager/HOReviewer/RiskManager/FinalApprover/SystemAdmin) — Loan Officers could see Approve/Reject guarantor buttons post-submission. Fix 2: Approval Gate feature design fully deliberated and approved — per-stage `WorkflowApprovalGates` config (strict/flexible per BranchReview/HOReview/CreditAnalysis/FinalApproval), `ApprovalOverrideRecord` entity spec, gate behaviour (rejected = mandatory note or hard block; pending = soft warning or hard block), inline rejection notes on all item types, persistent override banners with "Resolved" state, replacement document flow. Memory saved to `project_approval_gate.md`. No code written — implementation is next task. |
| 7.2 | 2026-04-29 | Factory AI | Session 50: Configurable `CollateralTypeConfig` full stack — domain `Entity` (not AggregateRoot; `Entity` has no DomainEvents), `CollateralTypeConfigRepository`, EF config (`CollateralTypeConfigs` table, unique index on `Code`), 9 new columns on `Collaterals` table (`IndicativeValue_Amount/Currency`, `ValuationBasis`, `ValuerAcceptableValue_Amount/Currency`, `ValuerName`, `ValuerCompany`, `ValuationReportPath`, `CollateralTypeConfigId`), migration `20260429190713_AddCollateralTypeConfig` with 11 seeded default types (haircut 0–50%, MarketValue/FSV basis). Application: `CollateralTypeConfigCommands.cs` (Create/Update/Toggle handlers), `GetCollateralTypeConfigsHandler` (AllActive flag), `CollateralDtos.cs` updated (new `CollateralTypeConfigDto`, extended `CollateralDto`, updated `AddCollateralRequest`/`SetCollateralValuationRequest`). DI: `ICollateralTypeConfigRepository` + 5 handler registrations added. UI: `CollateralTypes.razor` admin page at `/admin/collateral-types` (table + inline add/edit modal, haircut as % ÷100 before save, Code read-only on edit, toggle active). NavMenu updated. `AddCollateralModal` rewritten — dynamic type dropdown from DB, `OnTypeConfigChanged` sets both Code and ConfigId, haircut info callout, indicative value field only (formal valuation moved to Credit flow), enum fallback. `SetCollateralValuationModal` updated — ValuationBasis selector, AcceptableValue = ValuationBasis × (1 − HaircutRate), valuer name/company fields, pre-populate from existing data. `SetCollateralValuationAsync` signature changed to `SetCollateralValuationRequest` object. Bug fix: 5 missing DI handler registrations caused silent `try/catch` empty returns in `ApplicationService` — seeded data not showing was root cause. `CanManageValuation` in `Detail.razor` now role-gated (CreditOfficer/CreditManager/HOReviewer/SystemAdmin) — Loan Officers excluded from valuation and valuation upload buttons. Build: Infrastructure 0 errors, Web.Intranet 0 errors. |
| 7.1 | 2026-04-14 | Factory AI | Session 48: Three UI bug fixes — (1) `StateHasChanged()` added to `LoadApplication()` in `Detail.razor` (fixes stale-state after all action handlers including committee voting, covers all 20+ callers uniformly); (2) document viewer modal in `DocumentsTab.razor` height increased to `height: 92vh; max-height: 92vh`; body replaced `height: 70vh` with `flex: 1; overflow: hidden; min-height: 0` to fill space between header and footer; (3) `SetupCommitteeModal.razor` structural fix — `show` class added to `.modal-backdrop`, `.modal` div nested inside backdrop (was sibling root), `@onclick:stopPropagation` added on `.modal`, missing closing `</div>` for backdrop added. Business proposal documents created: `docs/CRMS_Proposal.md` (24-slide PPT-ready Markdown, `---` slide separators, full lifecycle flow, AI scoring, compliance, tech, security, implementation, support, CTA) and `docs/CRMS_Proposal.html` (fully standalone HTML with embedded CSS — animated score bars, lifecycle stages with arrows, role cards, implementation timeline, SLA cards, dark hero section, print media query, all placeholders pre-filled). No backend changes, no migrations. |
| 7.0 | 2026-04-13 | Factory AI | Session 47: Offer Acceptance operational fields (CBN CP-compliant) — `OfferAcceptanceMethod` enum (InBranchSigning/Courier/Electronic), `CustomerSignedAt`, `KfsAcknowledged` added to `LoanApplication` aggregate, `ConfirmOfferAcceptanceCommand`, `ApplicationService`, `Detail.razor` modal, and `LoanApplicationConfiguration`; migration `AddOfferAcceptanceFields` created. Evidence viewer in `OfferAcceptanceTab.razor` (92vh iframe modal). Loan Pack PDF full data: `LoanPackData` extended with `ApprovedAmount/Tenor/Rate`, `CommitteeDecisionData`, `CommitteeMemberVoteData`; `GenerateLoanPackCommand.BuildLoanPackDataAsync` comprehensively rewritten (product name lookup, party bureau cross-reference, real customer profile, correct `BalanceSheet` property names, non-nullable decimal YoY growth, `AccountStatus.Performing` for active loans, `committeeReview.FinalDecision`); `IBureauReportRepository`+`BureauReportRepository` get `GetByLoanApplicationIdWithDetailsAsync`. `LoanPackPdfGenerator.cs`: approved terms in Executive Summary, Active Facilities + Delinquent Accounts tables in bureau section, new Committee Decision section (coloured banner, vote tally, recommended terms, member votes table). `CanGeneratePack` in `Detail.razor`: expanded from 3 statuses to all active statuses; role check added (LoanOfficer/CreditOfficer/HOReviewer/BranchApprover/FinalApprover/Operations/SystemAdmin + committee member). Build: 0 errors. |
