...
public partial class ApplicationService
{
    ...
    public async Task<List<BureauReportInfo>> GetBureauReportsAsync(Guid loanApplicationId)
    {
        try
        {
            ...
            return result.Data.Select(r => new BureauReportInfo
            {
                ...
                IsScoreDerived = r.ScoreGrade == "DERIVED",
                TotalLoans = r.TotalAccounts,
                ActiveLoans = r.ActiveLoans,
                PerformingLoans = r.PerformingAccounts,
                ClosedLoans = r.ClosedAccounts,
                TotalExposure = r.TotalOutstandingBalance,
                HighestFacility = r.TotalCreditLimit,
    ...
    public async Task<ApiResponse> ForceRerunCreditChecksAsync(Guid applicationId, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.CreditBureau.Commands.ProcessLoanCreditChecksHandler>();
            var result = await handler.Handle(
                new CRMS.Application.CreditBureau.Commands.ProcessLoanCreditChecksCommand(applicationId, userId, ForceRefresh: true),
                CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse.Ok()
                : ApiResponse.Fail(result.Error ?? "Failed to force re-run credit checks");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error force-rerunning credit checks for application {Id}", applicationId);
            return ApiResponse.Fail("Failed to force re-run credit checks: " + ex.Message);
        }
    }
    ...
    public async Task<ApiResponse> RefetchInternalBankStatementAsync(Guid applicationId, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.StatementAnalysis.Commands.RefetchInternalBankStatementHandler>();
            var result = await handler.Handle(new CRMS.Application.StatementAnalysis.Commands.RefetchInternalBankStatementCommand(applicationId, userId), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to re-fetch statement");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error re-fetching internal bank statement for application {Id}", applicationId);
            return ApiResponse.Fail("Failed to re-fetch statement");
        }
    }
    ...
    public async Task<ApiResponse<CRMS.Web.Intranet.Models.CacLookupResult>> FetchCacDirectorsAsync(string rcNumber, string companyName, string companyType = "RC")
    {
        ...
        {
            var smartComply = _sp.GetRequiredService<CRMS.Domain.Interfaces.ISmartComplyProvider>();
            var result = await smartComply.VerifyCacAdvancedAsync(rcNumber.Trim(), companyName, companyType);
            if (result.IsFailure)
    ...
    public async Task<CollateralDetailDto?> GetCollateralDetailAsync(Guid collateralId)
    {
        try
        {
            ...
            return new CollateralDetailDto
            {
                ...
                IndicativeValue = c.IndicativeValue,
                MarketValue = c.MarketValue,
                ForcedSaleValue = c.ForcedSaleValue,
                AcceptableValue = c.AcceptableValue,
                ValuerAcceptableValue = c.ValuerAcceptableValue,
                HaircutPercentage = c.HaircutPercentage,
                ValuationBasis = c.ValuationBasis,
                ValuerName = c.ValuerName,
                ValuerCompany = c.ValuerCompany,
                ValuationReportPath = c.ValuationReportPath,
                Currency = c.Currency,
                CollateralTypeConfigId = c.CollateralTypeConfigId,
            ...
            foreach (var summary in summaryResult.Data)
            {
                ...
                if (detailResult.IsSuccess && detailResult.Data != null)
                {
                    ...
                    items.Add(new CollateralInfo
                    {
                        ...
                        LastValuationDate = c.LastValuationDate,
                        RejectionReason = c.RejectionReason,
                        IsLegalCleared = c.IsLegalCleared,
                        LegalClearedAt = c.LegalClearedAt,
                        LegalClearanceNotes = c.LegalClearanceNotes
                    });
                }
                else
                {
    ...
    private async Task<List<FinancialStatementInfo>> GetFinancialStatementsForApplicationAsync(Guid applicationId)
    {
        try
        {
            ...
            return result.Data.Select((FinancialStatementSummaryDto fs) => new FinancialStatementInfo
            {
                ...
                RejectionReason = fs.RejectionReason
            }).ToList();
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResponse> ReplaceCommitteeMemberAsync(
        Guid reviewId, Guid oldMemberUserId, Guid newMemberUserId,
        string newMemberName, string reason, Guid performedByUserId)
    {
        try
        {
            using var scope = _sp.CreateScope();
            var handler = scope.ServiceProvider.GetRequiredService<ReplaceCommitteeMemberHandler>();
            var result = await handler.Handle(
                new ReplaceCommitteeMemberCommand(reviewId, oldMemberUserId, newMemberUserId, newMemberName, reason, performedByUserId),
                CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to replace member");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error replacing committee member in review {ReviewId}", reviewId);
            return ApiResponse.Fail("Failed to replace committee member");
        }
    }
        ...
        string? conditions, Guid userId)
    {
        try
        {
            ...
            var decisionResult = await decisionHandler.Handle(
    ...
    public async Task<StandingCommitteeInfo?> GetStandingCommitteeByTypeAsync(string committeeType, Guid? locationId = null)
    {
        ...
        {
            _logger.LogError(ex, "Error fetching standing committee for type {Type}", committeeType);
            return null;
        }
    }
    ...
    public async Task<ApiResponse<CollateralResult>> AddCollateralAsync(CRMS.Web.Intranet.Services.AddCollateralRequest request, Guid userId)
    {
        try
        {
            ...
            AddCollateralCommand command = new AddCollateralCommand(
                Type: Enum.Parse<CollateralType>(request.Type, ignoreCase: true),
                ...
                CollateralTypeConfigId: request.CollateralTypeConfigId,
                IndicativeValue: request.IndicativeValue,
    ...
    public async Task<ApiResponse> SetCollateralValuationAsync(Guid collateralId, SetCollateralValuationRequest request)
    {
        try
        {
            SetCollateralValuationHandler handler = _sp.GetRequiredService<SetCollateralValuationHandler>();
            SetCollateralValuationCommand command = new SetCollateralValuationCommand(
                collateralId,
                request.MarketValue,
                request.ForcedSaleValue,
                request.Currency,
                request.HaircutPercentage,
                request.ValuationBasis,
                request.ValuerAcceptableValue,
                request.ValuerName,
                request.ValuerCompany,
                request.ValuationReportPath);
            ApplicationResult<CollateralDto> result = await handler.Handle(command, CancellationToken.None);
    ...
    public async Task<ApiResponse<List<CollateralTypeConfigItem>>> GetCollateralTypeConfigsAsync()
    {
        try
        {
            var handler = _sp.GetRequiredService<GetCollateralTypeConfigsHandler>();
            var result = await handler.Handle(new GetCollateralTypeConfigsQuery(ActiveOnly: true), CancellationToken.None);
            if (!result.IsSuccess || result.Data == null)
                return ApiResponse<List<CollateralTypeConfigItem>>.Fail(result.Error ?? "Failed to load collateral types");

            return ApiResponse<List<CollateralTypeConfigItem>>.Ok(result.Data.Select(c => new CollateralTypeConfigItem
            {
                Id = c.Id,
                Name = c.Name,
                Code = c.Code,
                HaircutRate = c.HaircutRate,
                ValuationBasis = c.ValuationBasis
            }).ToList());
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error loading collateral type configurations");
            return ApiResponse<List<CollateralTypeConfigItem>>.Fail("Failed to load collateral types");
        }
    }

    public async Task<ApiResponse<List<CollateralTypeConfigDto>>> GetCollateralTypeConfigsAllAsync()
    {
        try
        {
            var handler = _sp.GetRequiredService<GetCollateralTypeConfigsHandler>();
            var result = await handler.Handle(new GetCollateralTypeConfigsQuery(ActiveOnly: false), CancellationToken.None);
            if (!result.IsSuccess || result.Data == null)
                return ApiResponse<List<CollateralTypeConfigDto>>.Fail(result.Error ?? "Failed to load collateral types");
            return ApiResponse<List<CollateralTypeConfigDto>>.Ok(result.Data);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error loading all collateral type configurations");
            return ApiResponse<List<CollateralTypeConfigDto>>.Fail("Failed to load collateral types");
        }
    }

    public async Task<ApiResponse> CreateCollateralTypeConfigAsync(CreateCollateralTypeConfigRequest request, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CreateCollateralTypeConfigHandler>();
            var result = await handler.Handle(new CreateCollateralTypeConfigCommand(
                request.Name, request.Code, request.HaircutRate, request.ValuationBasis, userId,
                request.Description, request.SortOrder), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to create collateral type");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating collateral type configuration");
            return ApiResponse.Fail("Failed to create collateral type");
        }
    }

    public async Task<ApiResponse> UpdateCollateralTypeConfigAsync(Guid id, UpdateCollateralTypeConfigRequest request, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<UpdateCollateralTypeConfigHandler>();
            var result = await handler.Handle(new UpdateCollateralTypeConfigCommand(
                id, request.Name, request.HaircutRate, request.ValuationBasis, userId,
                request.Description, request.SortOrder), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to update collateral type");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error updating collateral type configuration {Id}", id);
            return ApiResponse.Fail("Failed to update collateral type");
        }
    }

    public async Task<ApiResponse> ToggleCollateralTypeConfigAsync(Guid id, bool activate, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<ToggleCollateralTypeConfigHandler>();
            var result = await handler.Handle(new ToggleCollateralTypeConfigCommand(id, activate, userId), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to update collateral type status");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error toggling collateral type configuration {Id}", id);
            return ApiResponse.Fail("Failed to update collateral type status");
        }
    }

    public async Task<ApiResponse> RecordLegalClearanceAsync(Guid collateralId, Guid userId, string? notes)
    {
        try
        {
            var handler = _sp.GetRequiredService<RecordLegalClearanceHandler>();
            var result = await handler.Handle(new RecordLegalClearanceCommand(collateralId, userId, notes), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to record legal clearance");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error recording legal clearance for collateral {CollateralId}", collateralId);
            return ApiResponse.Fail("Failed to record legal clearance");
        }
    }
    ...
    public async Task<ApiResponse> DeleteDocumentAsync(Guid applicationId, Guid documentId)
    {
        try
        {
            var handler = _sp.GetRequiredService<DeleteDocumentHandler>();
            var result = await handler.Handle(new DeleteDocumentCommand(applicationId, documentId), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to delete document");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting document");
            return ApiResponse.Fail("Failed to delete document");
        }
    }
        ...
        DateTime customerSignedAt, OfferAcceptanceMethod acceptanceMethod, bool kfsAcknowledged)
    {
        try
        {
            ...
            // 2. Transition workflow to OfferAccepted
            var workflowInstance = await GetWorkflowInstanceByApplicationIdAsync(applicationId);
            if (workflowInstance == null)
                return ApiResponse.Fail("Workflow instance not found for this application");

            var acceptResult = await TransitionWorkflowAsync(workflowInstance.Id, LoanApplicationStatus.OfferAccepted,
                WorkflowAction.MoveToNextStage, comments, userId, userRole);
            if (!acceptResult.Success)
                return acceptResult;

            // 3. Automatically advance to SecurityPerfection (system-triggered, same pattern as BranchApproved→CreditAnalysis)
            using var secScope = _sp.CreateScope();
            var secHandler = secScope.ServiceProvider.GetRequiredService<Application.LoanApplication.Commands.MoveToSecurityPerfectionHandler>();
            var secResult = await secHandler.Handle(new Application.LoanApplication.Commands.MoveToSecurityPerfectionCommand(applicationId, userId), CancellationToken.None);
            if (!secResult.IsSuccess)
                return ApiResponse.Fail(secResult.Error ?? "Failed to advance to security perfection");

            // 4. Reload workflow and transition to SecurityPerfection
            workflowInstance = await GetWorkflowInstanceByApplicationIdAsync(applicationId);
            if (workflowInstance == null)
                return ApiResponse.Fail("Workflow instance not found after offer acceptance");

            return await TransitionWorkflowAsync(workflowInstance.Id, LoanApplicationStatus.SecurityPerfection,
                WorkflowAction.MoveToNextStage, null, userId, Roles.SystemAdmin);
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResponse> DisburseApplicationAsync(Guid applicationId, Guid userId, string userRole, string coreBankingLoanId, string? comments)
    {
        try
        {
            ...
            return await TransitionWorkflowAsync(workflowInstance.Id, LoanApplicationStatus.Disbursed,
    ...
    public async Task<ApiResponse> ApproveApplicationAsync(Guid applicationId, string? comments, Guid userId, string userRole)
    {
        try
        {
            ...
            {
                using var domainScope = _sp.CreateScope();
                var moveToLegalHandler = domainScope.ServiceProvider
                    .GetRequiredService<Application.LoanApplication.Commands.MoveToLegalReviewHandler>();
                var domainResult = await moveToLegalHandler.Handle(
                    new Application.LoanApplication.Commands.MoveToLegalReviewCommand(applicationId, userId),
                    CancellationToken.None);
                if (!domainResult.IsSuccess)
                    return ApiResponse.Fail(domainResult.Error ?? "Move to legal review failed");
            }
            else if (currentStatus == "LegalReview")
            {
                using var domainScope = _sp.CreateScope();
                var submitOpinionHandler = domainScope.ServiceProvider
                    .GetRequiredService<Application.LoanApplication.Commands.SubmitLegalOpinionHandler>();
                var domainResult = await submitOpinionHandler.Handle(
                    new Application.LoanApplication.Commands.SubmitLegalOpinionCommand(applicationId, userId, comments),
                    CancellationToken.None);
                if (!domainResult.IsSuccess)
                    return ApiResponse.Fail(domainResult.Error ?? "Submit legal opinion failed");
            }
            else if (currentStatus == "LegalApproval")
            {
                using var domainScope = _sp.CreateScope();
                var approveLegalHandler = domainScope.ServiceProvider
                    .GetRequiredService<Application.LoanApplication.Commands.ApproveLegalReviewHandler>();
                var domainResult = await approveLegalHandler.Handle(
                    new Application.LoanApplication.Commands.ApproveLegalReviewCommand(applicationId, userId, comments),
                    CancellationToken.None);
                if (!domainResult.IsSuccess)
                    return ApiResponse.Fail(domainResult.Error ?? "Legal approval failed");
            }
            ...
            {
                using var domainScope = _sp.CreateScope();
                var h = domainScope.ServiceProvider.GetRequiredService<Application.LoanApplication.Commands.ApproveSecurityPerfectionHandler>();
                var domainResult = await h.Handle(new Application.LoanApplication.Commands.ApproveSecurityPerfectionCommand(applicationId, userId, comments), CancellationToken.None);
                if (!domainResult.IsSuccess)
                    return ApiResponse.Fail(domainResult.Error ?? "Security perfection approval failed");
            }
            else if (currentStatus == "DisbursementPending")
            {
                using var domainScope = _sp.CreateScope();
                var h = domainScope.ServiceProvider.GetRequiredService<Application.LoanApplication.Commands.PrepareDisbursementMemoHandler>();
                var domainResult = await h.Handle(new Application.LoanApplication.Commands.PrepareDisbursementMemoCommand(applicationId, userId, comments), CancellationToken.None);
                if (!domainResult.IsSuccess)
    ...
    public async Task<ApiResponse> ReturnApplicationAsync(Guid applicationId, string comments, Guid userId, string userRole)
    {
        try
        {
            ...
            else if (currentStatus == "LegalReview")
            {
                using var domainScope = _sp.CreateScope();
                var returnHandler = domainScope.ServiceProvider
                    .GetRequiredService<Application.LoanApplication.Commands.ReturnFromLegalReviewHandler>();
                var domainResult = await returnHandler.Handle(
                    new Application.LoanApplication.Commands.ReturnFromLegalReviewCommand(applicationId, userId, comments),
                    CancellationToken.None);
                if (!domainResult.IsSuccess)
                    return ApiResponse.Fail(domainResult.Error ?? "Return from legal review failed");
            }
            else if (currentStatus == "LegalApproval")
            {
                using var domainScope = _sp.CreateScope();
                var returnHandler = domainScope.ServiceProvider
                    .GetRequiredService<Application.LoanApplication.Commands.ReturnFromLegalApprovalHandler>();
                var domainResult = await returnHandler.Handle(
                    new Application.LoanApplication.Commands.ReturnFromLegalApprovalCommand(applicationId, userId, comments),
                    CancellationToken.None);
                if (!domainResult.IsSuccess)
            ...
            {
                using var domainScope = _sp.CreateScope();
                var h = domainScope.ServiceProvider.GetRequiredService<Application.LoanApplication.Commands.ReturnFromSecurityApprovalHandler>();
                var domainResult = await h.Handle(new Application.LoanApplication.Commands.ReturnFromSecurityApprovalCommand(applicationId, userId, comments), CancellationToken.None);
                if (!domainResult.IsSuccess)
    ...
    public async Task<ApiResponse<DisbursementChecklistModel>> GetDisbursementChecklistAsync(Guid applicationId)
    {
        try
        {
            ...
            // Build a name lookup for any waiver proposers — avoids N+1 calls
            var waiverProposerIds = dto.Items
                .Where(i => i.WaiverProposedByUserId.HasValue)
                .Select(i => i.WaiverProposedByUserId!.Value)
                .Distinct()
                .ToHashSet();
            var userNameLookup = new Dictionary<Guid, string>();
            if (waiverProposerIds.Count > 0)
            {
                var users = await GetUsersAsync();
                userNameLookup = users
                    .Where(u => waiverProposerIds.Contains(u.Id))
            ...
            return ApiResponse<DisbursementChecklistModel>.Ok(new DisbursementChecklistModel
            {
                ...
                Items = dto.Items.Select(i => new ChecklistItemModel
                {
                    ...
                    WaiverProposedByUserName = i.WaiverProposedByUserId.HasValue && userNameLookup.TryGetValue(i.WaiverProposedByUserId.Value, out var pName) ? pName : null,
    ...
    public async Task<ApiResponse> VerifyFinancialStatementAsync(Guid statementId, Guid userId, string? notes)
    {
        try
        {
            var handler = _sp.GetRequiredService<Application.FinancialAnalysis.Commands.VerifyFinancialStatementHandler>();
            var result = await handler.Handle(
                new Application.FinancialAnalysis.Commands.VerifyFinancialStatementCommand(statementId, userId, notes),
                CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error!);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error verifying financial statement {Id}", statementId);
            return ApiResponse.Fail("Failed to verify financial statement");
        }
    }

    public async Task<ApiResponse> RejectFinancialStatementAsync(Guid statementId, string reason)
    {
        try
        {
            var handler = _sp.GetRequiredService<Application.FinancialAnalysis.Commands.RejectFinancialStatementHandler>();
            var result = await handler.Handle(
                new Application.FinancialAnalysis.Commands.RejectFinancialStatementCommand(statementId, reason),
                CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error!);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error rejecting financial statement {Id}", statementId);
            return ApiResponse.Fail("Failed to reject financial statement");
        }
    }

    // ─── Approval Gate ────────────────────────────────────────────────────────────

    public async Task<ApprovalGateResultModel?> CheckApprovalGateAsync(Guid applicationId, string stage)
    {
        try
        {
            var handler = _sp.GetRequiredService<Application.Workflow.Queries.CheckApprovalGateHandler>();
            var result = await handler.Handle(
                new Application.Workflow.Queries.CheckApprovalGateQuery(applicationId, stage),
                CancellationToken.None);
            if (!result.IsSuccess || result.Data == null) return null;

            return new ApprovalGateResultModel
            {
                IsStrict = result.Data.IsStrict,
                RejectedItems = result.Data.RejectedItems.Select(x => new GateItemModel
                {
                    ItemId = x.ItemId,
                    ItemType = x.ItemType,
                ...
                PendingItems = result.Data.PendingItems.Select(x => new GateItemModel
                {
                    ItemId = x.ItemId,
                    ItemType = x.ItemType,
                    ItemLabel = x.ItemLabel,
        ...
        {
            _logger.LogError(ex, "Error checking approval gate for application {Id}", applicationId);
            return null;
        }
    }

    public async Task<ApiResponse> SaveApprovalOverrideAsync(Guid applicationId, string stage, Guid actorId, string actorName, string noteText)
    {
        try
        {
            var handler = _sp.GetRequiredService<Application.Workflow.Commands.SaveApprovalOverrideHandler>();
            var result = await handler.Handle(
                new Application.Workflow.Commands.SaveApprovalOverrideCommand(applicationId, stage, actorId, actorName, noteText),
                CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error!);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error saving approval override for application {Id}", applicationId);
            return ApiResponse.Fail("Failed to save approval override note");
        }
    }

    public async Task<List<ApprovalOverrideInfo>> GetApprovalOverridesAsync(Guid applicationId)
    {
        try
        {
            var handler = _sp.GetRequiredService<Application.Workflow.Commands.GetApprovalOverridesHandler>();
            var result = await handler.Handle(
                new Application.Workflow.Commands.GetApprovalOverridesQuery(applicationId),
                CancellationToken.None);
            if (!result.IsSuccess || result.Data == null) return [];

            return result.Data.Select(r => new ApprovalOverrideInfo
            {
                Id = r.Id,
                Stage = r.Stage,
    ...
    // ── NAMP Queue & Applications ─────────────────────────────────────────

    public async Task<List<CRMS.Application.Namp.DTOs.NampStagingRecordSummaryDto>> GetNampStagingQueueAsync(Guid? branchId = null)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Queries.GetNampStagingQueueHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Queries.GetNampStagingQueueQuery(branchId), CancellationToken.None);
            return result.IsSuccess && result.Data != null ? result.Data : [];
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error loading NAMP staging queue");
            return [];
        }
    }
    ...
    public async Task<List<CRMS.Application.Namp.DTOs.NampApplicationSummaryDto>> GetNampApplicationsByStatusAsync(string status, Guid? branchId = null)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Queries.GetNampApplicationsByStatusHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Queries.GetNampApplicationsByStatusQuery(status, branchId), CancellationToken.None);
            return result.IsSuccess && result.Data != null ? result.Data : [];
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error loading NAMP applications by status {Status}", status);
            return [];
        }
    }

    public async Task<List<CRMS.Application.Namp.DTOs.NampApplicationSummaryDto>> GetNampApplicationsByStatusAndTierAsync(string status, string tier, Guid? branchId = null)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Queries.GetNampApplicationsByStatusAndTierHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Queries.GetNampApplicationsByStatusAndTierQuery(status, tier, branchId), CancellationToken.None);
            return result.IsSuccess && result.Data != null ? result.Data : [];
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error loading NAMP applications by status {Status} tier {Tier}", status, tier);
            return [];
        }
    }

    public async Task<List<CRMS.Application.Namp.DTOs.NampApplicationSummaryDto>> GetNampApplicationsByStatusesAsync(IEnumerable<string> statuses, Guid? branchId = null)
    {
        var results = new List<CRMS.Application.Namp.DTOs.NampApplicationSummaryDto>();
        foreach (var s in statuses)
            results.AddRange(await GetNampApplicationsByStatusAsync(s, branchId));
    ...
    public async Task<CRMS.Application.Namp.DTOs.NampApplicationDto?> GetNampApplicationByIdAsync(Guid id)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Queries.GetNampApplicationByIdHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Queries.GetNampApplicationByIdQuery(id), CancellationToken.None);
            return result.IsSuccess ? result.Data : null;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error loading NAMP application {Id}", id);
            return null;
        }
    }

    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> RecallNampApplicationAsync(Guid stagingId, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.RecallNampApplicationHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.RecallNampApplicationCommand(stagingId, userId), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Recall failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error recalling NAMP staging record {Id}", stagingId);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to recall application: {ex.Message}");
        }
    }

    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> SubmitNampApplicationAsync(Guid id, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.SubmitNampApplicationHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.SubmitNampApplicationCommand(id, userId), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Submit failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error submitting NAMP application {Id}", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to submit application: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> SubmitNampFinancialAppraisalAsync(Guid id, Guid userId, bool isApproved, string? note)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.SubmitNampFinancialAppraisalHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.SubmitNampFinancialAppraisalCommand(id, userId, isApproved, note), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Financial appraisal failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error submitting NAMP financial appraisal {Id}", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to submit financial appraisal: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> CirculateNampToCommitteeAsync(
        ...
        int requiredVotes, int minApprovalVotes, int deadlineHours)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.CirculateNampToCommitteeHandler>();
            var result = await handler.Handle(
                new CRMS.Application.Namp.Commands.CirculateNampToCommitteeCommand(id, userId, members, requiredVotes, minApprovalVotes, deadlineHours),
                CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Committee circulation failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error circulating NAMP application {Id} to committee", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to circulate to committee: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> DeclineNampRatificationAsync(Guid id, Guid userId, string? note)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.DeclineNampRatificationHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.DeclineNampRatificationCommand(id, userId, note), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Ratification decline failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error declining NAMP ratification {Id}", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to decline ratification: {ex.Message}");
        }
    }

    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> RecordNampOfferAcceptanceAsync(Guid id, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.RecordNampOfferAcceptanceHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.RecordNampOfferAcceptanceCommand(id, userId), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Failed to record offer acceptance");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error recording NAMP offer acceptance {Id}", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to record offer acceptance: {ex.Message}");
        }
    }

    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> LapseNampOfferAsync(Guid id, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.LapseNampOfferHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.LapseNampOfferCommand(id, userId), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Failed to lapse offer");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error lapsing NAMP offer {Id}", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to lapse offer: {ex.Message}");
        }
    }

    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> BeginNampPreDeploymentVerificationAsync(Guid id, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.BeginNampPreDeploymentVerificationHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.BeginNampPreDeploymentVerificationCommand(id, userId), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Failed to begin pre-deployment verification");
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> CompleteNampPreDeploymentVerificationAsync(Guid id, Guid userId, string? note)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.CompleteNampPreDeploymentVerificationHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.CompleteNampPreDeploymentVerificationCommand(id, userId, note), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Failed to complete pre-deployment verification");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error completing NAMP pre-deployment verification {Id}", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to complete pre-deployment verification: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> ConfirmNampDeploymentAsync(Guid id, Guid userId, bool gpsActivated, string? note)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.ConfirmNampDeploymentHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.ConfirmNampDeploymentCommand(id, userId, gpsActivated, note), CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Failed to confirm deployment");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error confirming NAMP deployment {Id}", id);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail($"Failed to confirm deployment: {ex.Message}");
        }
    }
    ...
    public async Task<List<CRMS.Application.Namp.DTOs.NampRoutingConfigDto>> GetNampRoutingConfigsAsync()
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Queries.GetNampRoutingConfigsHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Queries.GetNampRoutingConfigsQuery(), CancellationToken.None);
            return result.IsSuccess && result.Data != null ? result.Data : [];
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error loading NAMP routing configs");
            return [];
        }
    }

    public async Task<ApiResponse> CreateNampRoutingConfigAsync(string applicantCategory, string committeeTier, decimal minValue, decimal maxValue, int priority, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.CreateNampRoutingConfigHandler>();
            var result = await handler.Handle(
                new CRMS.Application.Namp.Commands.CreateNampRoutingConfigCommand(applicantCategory, committeeTier, minValue, maxValue, priority, userId),
                CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to create routing config");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating NAMP routing config");
            return ApiResponse.Fail("Failed to create routing config");
        }
    }

    public async Task<ApiResponse> UpdateNampRoutingConfigAsync(Guid id, decimal minValue, decimal maxValue, int priority, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.UpdateNampRoutingConfigHandler>();
            var result = await handler.Handle(
                new CRMS.Application.Namp.Commands.UpdateNampRoutingConfigCommand(id, minValue, maxValue, priority, userId),
                CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to update routing config");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error updating NAMP routing config {Id}", id);
            return ApiResponse.Fail("Failed to update routing config");
        }
    }

    public async Task<ApiResponse> ToggleNampRoutingConfigAsync(Guid id, bool activate, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.ToggleNampRoutingConfigHandler>();
            var result = await handler.Handle(
                new CRMS.Application.Namp.Commands.ToggleNampRoutingConfigCommand(id, activate, userId),
                CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Failed to toggle routing config");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error toggling NAMP routing config {Id}", id);
    ...
    public async Task<List<CRMS.Application.Namp.DTOs.NampWorkflowConfigDto>> GetNampWorkflowConfigsAsync()
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Queries.GetNampWorkflowConfigsHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Queries.GetNampWorkflowConfigsQuery(), CancellationToken.None);
            return result.IsSuccess && result.Data != null ? result.Data : [];
        }
        catch (Exception ex)
        {
    ...
    public async Task<ApiResponse> UpdateNampDocumentAsync(Guid documentId, Guid userId, string category, string? description)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.UpdateNampDocumentHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.UpdateNampDocumentCommand(documentId, userId, category, description), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Update failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error updating NAMP document {DocId}", documentId);
            return ApiResponse.Fail(ex.Message);
        }
    }

    public async Task<ApiResponse> DeleteNampDocumentAsync(Guid nampApplicationId, Guid documentId, Guid userId)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.DeleteNampDocumentHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.DeleteNampDocumentCommand(nampApplicationId, documentId, userId), CancellationToken.None);
            return result.IsSuccess ? ApiResponse.Ok() : ApiResponse.Fail(result.Error ?? "Delete failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deleting NAMP document {DocId}", documentId);
            return ApiResponse.Fail(ex.Message);
        }
    }

    public async Task<ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>> UploadNampDocumentAsync(
        Guid nampApplicationId, Guid userId, string stage, string fileName, string contentType,
        long fileSize, Stream fileContent, string category = "General", string? description = null)
    {
        try
        {
            var fileStorage = _sp.GetRequiredService<CRMS.Domain.Interfaces.IFileStorageService>();
            string containerName = $"namp/{nampApplicationId}/documents";
            string storagePath = await fileStorage.UploadAsync(containerName, fileName, fileContent, contentType);

            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.UploadNampDocumentHandler>();
            var result = await handler.Handle(new CRMS.Application.Namp.Commands.UploadNampDocumentCommand(
                nampApplicationId, userId, stage, fileName, contentType, fileSize, storagePath, category, description),
                CancellationToken.None);
            return result.IsSuccess
                ? ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Ok(result.Data!)
                : ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(result.Error ?? "Upload failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error uploading NAMP document for application {Id}", nampApplicationId);
            return ApiResponse<CRMS.Application.Namp.DTOs.NampApplicationDto>.Fail(ex.Message);
        }
    }
        ...
        decimal? utilisationRateAssumption = null, string? demandEvidenceNote = null)
    {
        try
        {
            var handler = _sp.GetRequiredService<CRMS.Application.Namp.Commands.SaveNampFinancialAppraisalReportHandler>();
            ...
            return result.IsSuccess
                ...
                : ApiResponse<CRMS.Application.Namp.DTOs.NampFinancialAppraisalReportDto>.Fail(result.Error ?? "Save failed");
        }
        catch (Exception ex)
        {
